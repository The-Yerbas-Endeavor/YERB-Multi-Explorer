  ...
  for (const event of events) {
    ...
    if (event.type === 'issue' || event.type === 'reissue') {
      await Asset.findOneAndUpdate(
        ...
        {
          $set: {
            normalizedName: String(event.asset).toUpperCase(),
            amount: event.amount,
            units: event.units,
            reissuable: event.reissuable,
            hasIpfs: Boolean(event.ipfs_hash),
            ipfsHash: event.ipfs_hash,
            issuanceTxid: tx.txid,
            issuanceHeight: block.height,
            metadata: event
          }
        },
        { upsert: true }
...
const worker = new Worker(
  ...
  async job => {
    ...
    await SyncState.updateOne({ key: 'blocks' }, { height, hash, status: 'indexed', error: null }, { upsert: true });

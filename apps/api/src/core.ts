...
const TransactionSchema = new Schema({
  ...
  valueOut: Number,
  fees: Number,
  vin: [Schema.Types.Mixed],
  vout: [Schema.Types.Mixed],
  assetEvents: [Schema.Types.Mixed],
  raw: Schema.Types.Mixed
}, { timestamps: true });
...
const AddressSchema = new Schema({
  ...
  assetBalances: { type: Map, of: Number, default: {} },
...
export async function enqueueMissingBlocks(): Promise<number> {
  ...
  for (let height = state.height + 1; height <= upper; height += 1) {
    await blockQueue.add('index-block', { height }, { jobId: `block:${height}`, removeOnComplete: 1000, attempts: 5, backoff: { type: 'exponential', delay: 2000 } });
    added += 1;
  }

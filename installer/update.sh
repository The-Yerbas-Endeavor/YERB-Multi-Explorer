...
if [[ -d "$APP_DIR/modules/assets-viewer" && -f "$APP_DIR/installer/integrate-assets.sh" ]]; then
  chmod 0755 "$APP_DIR/installer/integrate-assets.sh"
...
curl -fsS http://127.0.0.1/assets/api/?resource=status >/dev/null 2>&1 || true
